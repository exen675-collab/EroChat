...
interface CharacterCardImportFile {
    originalname?: string;
    mimetype?: string;
    buffer: Buffer;
}
...
function createImportError(message: string, statusCode = 400): ImportError {
...
function parseCardJsonText(payloadText: string) {
...
function validateCharacterCard(card: unknown): CharacterCard {
    ...
    return {
        ...
        data: card.data as CharacterCard['data']
    };
}

function decodeTextChunk(data: Buffer): TextChunk | null {
...
function decodeCompressedTextChunk(data: Buffer): TextChunk | null {
    const separatorIndex = data.indexOf(0);
...
function decodeInternationalTextChunk(data: Buffer): TextChunk | null {
...
function extractCardPayloadFromPng(buffer: Buffer): string {
    const chunks = parsePngChunks(buffer);

    for (const chunk of chunks) {
        let entry: TextChunk | null = null;
        ...
        if (
            String(entry.keyword || '')
                .trim()
                .toLowerCase() !== CHARACTER_CARD_KEYWORD
        ) {
            continue;
        }
...
function parseCharacterCardJsonBuffer(buffer: Buffer): CharacterCard {
    const rawText = buffer.toString('utf8').replace(/^\uFEFF/, '');
    return validateCharacterCard(parseCardJsonText(rawText));
}

function parseCharacterCardPngBuffer(buffer: Buffer): CharacterCard {
...
function parseCharacterCardImportFile(file: CharacterCardImportFile) {
